...
AGGREGATION_THRESHOLD = 1  # Updates instantly for demo
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Cyber Shield Backend...")
...
async def aggregate_gradients() -> Dict[str, Any]:
    async with aggregation_lock:
        try:
            ...
            # STRICT FLOAT CASTING FOR MONGODB COMPATIBILITY
...
async def process_live_transaction(data: TransactionInput, company: dict = Depends(verify_api_key)):
    ...
    return {
        ...
        "message": "Fraudulent pattern detected by Cyber Shield Network" if is_fraud else "Transaction looks secure"
    }

# --- THE SDK SCRIPT ---
@api_router.get("/client/sdk")
async def get_client_sdk(request: Request):
    api_url = f"{str(request.base_url).rstrip('/')}/api"
    
    sdk_content = f'''"""
FinSecure SDK v2.0
The official Python library for connecting to the Cyber Shield Federated Network.
"""
import requests
import numpy as np
import base64
...
class FinSecureClient:
    def __init__(self, api_key, server_url="{api_url}"):
        self.api_key = api_key
        self.server_url = server_url.rstrip('/')
        self.headers = {{"X-API-Key": self.api_key}}
        self.current_round = 0
        
        print(f"🔒 Cyber Shield SDK Initialized")
        print(f"   Server: {{self.server_url}}")

    def connect(self):
        try:
            print("   Connecting...", end=" ", flush=True)
            response = requests.get(f"{{self.server_url}}/auth/verify", headers=self.headers)
    ...
    def fetch_global_model(self):
        try:
            response = requests.get(f"{{self.server_url}}/model/download", headers=self.headers)
            if response.status_code == 200:
                data = response.json()
    ...
    def submit_update(self, model, X_train_len, metrics):
        try:
            ...
            response = requests.post(
                f"{{self.server_url}}/federated/submit-gradients",
...
# --- FULL INTERACTIVE TERMINAL NODE SCRIPT ---
...
"""
Cyber Shield Interactive Gateway Terminal
...
while True:
    ...
    if choice == '1':
        ...
        with open(ML_BUFFER_FILE, 'a', newline='') as f:
            writer = csv.writer(f)
            # Create 100 noisy variations of the exact reported fraud
            for _ in range(100):
                noisy_txn = list(last_txn)
                noisy_txn[0] = round(noisy_txn[0] * np.random.uniform(0.8, 1.2), 2)
                noisy_txn[4] = round(noisy_txn[4] * np.random.uniform(0.8, 1.2), 1)
        ...
        try:
            ...
            # Pull the most recent 100 safe transactions from the bank's ledger
            safe_data = core_data[core_data[:, 10] == 0][-100:]
            # Pull the 100 fraud variations
            fraud_data = np.genfromtxt(ML_BUFFER_FILE, delimiter=',', skip_header=1)[-100:]
            ...
            # Train heavily for 30 epochs so the model genuinely learns this specific threat
            hist = model.fit(X_train, y_train, epochs=30, verbose=0)
...
async def submit_gradients(gradient_submit: GradientSubmit, request: Request, company: dict = Depends(verify_api_key)):
    ...
    if pending_count >= AGGREGATION_THRESHOLD:
        ...
        # Await it directly so it calculates instantly
...
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])from fastapi import FastAPI, APIRouter, HTTPException, Header, Depends, Request
from fastapi.responses import PlainTextResponse
from dotenv import load_dotenv
from starlette.middleware.cors import CORSMiddleware
from motor.motor_asyncio import AsyncIOMotorClient
